...
export default function EditFAQPage() {
  ...
  useEffect(() => {
    const fetchFAQ = async () => {
      ...
      try {
        ...
        const response = await fetch(`/api/admin/faqs/${encodeURIComponent(faqId)}`);
        
        if (!response.ok) {
          if (response.status === 404) {
            throw new Error(`FAQ with ID "${faqId}" not found`);
          }
          throw new Error(`Error fetching FAQ: ${response.status}`);
        }
        ...
        if (!data || !data.question) {
          throw new Error('Received invalid FAQ data from server');
        }
        ...
        setError(error instanceof Error ? error.message : 'Failed to load FAQ. Please try again.');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  // If there's an error related to not finding the FAQ, show a more user-friendly error page
  if (error && (error.includes("not found") || error.includes("missing") || error.includes("undefined"))) {
    return (
      <div className="container mx-auto py-8 px-4 max-w-4xl">
        <div className="mb-6">
          <Link href="/admin/chatbot?tab=faqs" className="text-blue-600 hover:text-blue-800 flex items-center">
        ...
        <div className="bg-white shadow-lg border border-gray-100 rounded-lg overflow-hidden">
          <div className="bg-red-600 px-6 py-4">
            <h1 className="text-2xl font-bold text-white">FAQ Not Found</h1>
          </div>
          ...
          <div className="p-8 text-center">
            ...
            <p className="text-gray-600 mb-6">
              The FAQ you're trying to edit doesn't exist or couldn't be loaded.
            </p>
            
            <div className="flex justify-center space-x-4">
              <Link href="/admin/chatbot?tab=faqs">
                <button className="px-4 py-2 bg-gray-200 text-gray-700 rounded hover:bg-gray-300 transition-colors duration-200">
                  Return to FAQs
                </button>
              </Link>
              
              <Link href="/admin/chatbot/faqs/new">
        ...
        <div className="p-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex justify-end space-x-3 pt-4 border-t border-gray-100">
              ...
              <button 
                type="submit" 
                disabled={isSubmitting || !question || !answer || keywords.length === 0}
