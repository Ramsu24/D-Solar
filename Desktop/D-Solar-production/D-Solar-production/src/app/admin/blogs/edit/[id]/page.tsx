  ...
  useEffect(() => {
    const fetchBlog = async () => {
      try {
        ...
        setAuthor(data.author || 'D-Solar Team');
        setCategory(data.category || 'Solar Energy');
        setTags(data.tags || []);
      } catch (err) {
    ...
    return (
      <div className="py-6">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-4 bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded flex items-start" role="alert">
            <LucideAlertCircle className="h-5 w-5 mr-2 mt-0.5" />
            <span>{error}</span>
            <button
              onClick={() => router.push('/admin/blogs')}
              className="mt-2 px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-red-600 hover:bg-red-700"
            >
              Back to Blogs
            </button>
          </div>
